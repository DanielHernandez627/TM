...
def search_Dataterminal(ae):#ae llega el verdad literal
    global table
    if(verify_table_select() == 1):
        table = "tablaIntroduccion.json"
    elif(verify_table_select() == 2):
...
def verify_Gramatic(size_terminal,x,ae):#size_terminal recorre toda la oracion
    i = 0
    exit_gramitc = ""
    global final_exit
    x_original = x
...
def analizador():
    ...
    for idx,x in enumerate(splitToken):
        terminal_select =  x
        if ((terminal_select == "desea") or (terminal_select == "gustaria") or (terminal_select == "ordenar") or (terminal_select == "repetir") or (terminal_select == "ver")):
            terminal_select = "V"
        splitToken2.pop(0)
        printer_pila_entrada()
